...
<html>
<head>
	<title>{{put[@data.vars.stuff.title>var)get_title]}}</title>
</head>
<body>
	<h1>{{put[@data.vars.stuff.title>var)get_title]}}</h1>
	<p>I have a few things and a number ({{put[@number]}}):</p>
	<ul>
	{{foreach @things as "item"}}
		<li>The val of item is {{put [@item>val, ", it has a few things of its own:"]}}
			<ul>
